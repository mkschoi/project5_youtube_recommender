...
def embedded_rec_videos(df):
    recs_video_ids = list(df['Video_ID'])
    recs_video_titles = list(df['Title'])
    recs_ids_titles = list(zip(recs_video_ids, recs_video_titles))
    
    for id, title in recs_ids_titles:
        st.subheader(title)
...
def follow_up_recommender(liked_video_list, df_videos_filtered):
    ...
    ## Drop videos that were in the initial recommendation
...
if session_state.search_button_init:
    ...
    embedded_rec_videos(df_videos_recs_init)
    ...
    video_id_follow_up = st.multiselect('Select one or more:', options=['', titles[0], titles[1], titles[2], titles[3], titles[4]])
    
    cross_intra_topic = st.radio('Select one:', ('Intra-topic', 'Cross-topic'))
    
    if video_id_follow_up: 
        search_button_follow_up = st.button('Search for recommended videos', key=2)
        if search_button_follow_up:
            if cross_intra_topic=='Intra-topic':
                df_videos_recs_follow_up = follow_up_recommender(video_id_follow_up, df_videos_filtered)
                embedded_rec_videos(df_videos_recs_follow_up)
            else:
                df_videos_recs_follow_up = follow_up_recommender(video_id_follow_up, df_videos_other)
                embedded_rec_videos(df_videos_recs_follow_up)
